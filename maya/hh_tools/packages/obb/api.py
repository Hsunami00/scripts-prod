...
class OBB(object):
    ...
    def getPoints(self, fnMesh):
        ...
        """
        #Re-declaring a lot of the needed types in order to support selected vertices.
        #This is redundant and will require a refactor of all the methods to clean up.
        
        #Convert mesh selections into verts
        if pm.nodeType(pm.ls(selection=True)[0]) == 'mesh':
            pm.select(pm.polyListComponentConversion(pm.ls(selection=True), toVertex=True)) 

        meshComponent  = OpenMaya.MObject()
        selDagPath = OpenMaya.MDagPath()
        selectList = OpenMaya.MSelectionList()
        OpenMaya.MGlobal.getActiveSelectionList(selectList)
        mPoints = OpenMaya.MPointArray()       
        
        selectList.getDagPath(0, selDagPath, meshComponent)
        vertIter = OpenMaya.MItMeshVertex(selDagPath, meshComponent)
        
        #If components, then only use what was selected.  Else, select all vertices.
        if not meshComponent.isNull():
            while not vertIter.isDone():
                mPoints.append(vertIter.position(OpenMaya.MSpace.kWorld))
                vertIter.next()
        else:
            fnMesh.getPoints(mPoints, OpenMaya.MSpace.kWorld)
